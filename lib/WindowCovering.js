	...
	if (currentPosition != value) {
		...
		if (currentPosition > value) {
			...
			if (this.config.orientation == 'vertical') {
				duration = this.config.duration_down * (currentPosition - value) / 100;
			}
			else {
				duration = this.config.duration * (currentPosition - value) / 100;
			}
			...
			if (this.config.orientation == 'vertical') {
				duration = this.config.duration_up * (currentPosition - value) / 100;
			}
			else {
				duration = this.config.duration * (value - currentPosition) / 100;
			}
...
// send up (vertical) or open (horizontal) command and wait 'duration' milliseconds before sending the stop command
WindowCovering.prototype.open = function(duration, callback) {
	if (this.config.orientation == 'vertical') {
		this._transmitter.up(this.config.deviceID);
	}
	else {
...
// send down (vertical) or close (horizontal) command and wait 'duration' milliseconds before sending the stop command
WindowCovering.prototype.close = function(duration, callback) {
	if (this.config.orientation == 'vertical') {
		this._transmitter.down(this.config.deviceID);
	}
	else {
